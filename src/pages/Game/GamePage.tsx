...
const tutorialSteps = [
  ...
  { kicker: '02 / CN + DN', title: '数据从哪里来，又要到哪里去', body: '上方入口来的货物会经过 CN，再送到下方三个 DN。操作重点在选项卡：选一个方案并确认，然后观察哪个 DN 变忙。', note: 'DN 负载就是节点当前有多忙；数字越高，余量越少。' },
  ...
  { kicker: '04 / SCORE MAP', title: '看懂四项评分和三行参数', body: '“查询 1 DN”看要问几座仓库，“搬运 10%”看要跨节点配合多少，“成本 9”看要占多少资源。把三行参数和任务卡上的分布、主要访问放在一起，先判断本波最怕什么，再用数字确认。', note: '查询、搬运、成本各自对应一种代价；任务卡会提示这一波该优先关注哪一项。' },
  ...
  { kicker: '09 / TRAINING 3', title: '训练 3 · 大型持续写入的综合判断', body: '先看任务卡的分布和主要访问，猜本波最需要避开的代价，再用查询、搬运、成本和 DN 余量检查判断。', note: '查询节点少还不够；大数据的额外代价也要算进去。' },
...
function DecisionDataGuide() {
  return <aside className="decision-data-guide" id="decision-data-guide" aria-label="决策数据说明">
    ...
    <footer>快速判断：先看任务卡的分布和主要访问，猜本波最怕绕路、拥堵还是浪费资源，再用查询、搬运、成本验证。最后检查 DN 余量，再做确认。</footer>
  </aside>
}
