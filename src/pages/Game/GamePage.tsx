...
function TutorialOptionPreview({ wave }: { wave: RankedWave }) {
  return <div className="tutorial-option-preview"><b>方案卡 / 参数预览</b>{wave.options.map((option) => <span key={option.id}><strong>{option.label}</strong><em>查询 {option.queryNodes} DN · 搬运 {option.crossNodeMovement}% · 成本 {option.resourceCost}</em></span>)}</div>
}
