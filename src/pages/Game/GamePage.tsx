...
const tutorialSteps = [
  ...
  { kicker: '03 / SHARD + QUERY', title: '放在哪里，会决定以后怎么查', body: '先读“分布”：地区或状态集中时，要优先避免某个 DN 过忙。再读“主要访问”：任务常按用户、订单或时间查时，要比较查询会问几个 DN。最后看规模和增长，它们决定成本要看得多重。', note: '读卡顺序：分布看负载，主要访问看查询，规模和增长看成本。' },
  { kicker: '04 / SCORE MAP', title: '看懂四项评分和三行参数', body: '“查询 1 DN”表示只问一座仓库，适合主要访问条件明确的任务；“搬运 10%”表示跨节点配合较少，分布越分散时越值得关注；“成本 9”表示资源占用较低，数据越大、增长越快时越关键。', note: '把这三行分别对照主要访问、分布、规模和增长，再结合 DN 余量做选择。' },
