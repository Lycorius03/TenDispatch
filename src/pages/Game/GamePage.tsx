...
function ModernConsole(p: GamePageProps & { displayLoads: GameState['dnLoads'] }) {
  ...
  return <main className="dispatch-console modern-console">
    ...
    {tutorialGuideStep && <><div className={`tutorial-spotlight is-${tutorialFocus}`} aria-hidden="true" /><aside className="tutorial-guide-dialog"><span>教学提示 · {tutorialSteps[s.tutorialStep - 1].kicker}</span><NPCChannel message={s.npcMessage} /></aside></>}
...
function TutorialTaskPreview({ wave }: { wave: RankedWave }) {
  return <div className="tutorial-task-preview"><b>任务卡 / 当前训练预览</b><span><strong>{wave.dataName}</strong> · 规模 {Array.from({ length: 5 }, (_, index) => index < wave.size ? '★' : '☆').join('')}</span><span>分布：{wave.distribution}</span><span>主要访问：{wave.access}{wave.publicData ? ' · 公共数据' : ''}</span></div>
}

function TutorialOptionPreview({ wave }: { wave: RankedWave }) {
  return <div className="tutorial-option-preview"><b>方案卡 / 先看数字，不用选择</b>{wave.options.map((option) => <span key={option.id}><strong>{option.label}</strong><em>查询 {option.queryNodes} DN · 搬运 {option.crossNodeMovement}% · 成本 {option.resourceCost}</em></span>)}</div>
}
