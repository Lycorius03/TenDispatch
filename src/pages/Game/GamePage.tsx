...
function TutorialMissionCard({ wave, waveIndex, loads, staticPreview = false }: { wave: RankedWave; waveIndex: number; loads: GameState['dnLoads']; staticPreview?: boolean }) {
  return <div className="modern-mission-copy" {...(staticPreview ? { 'data-tutorial-static': true } : {})}><span>TRAINING WAVE {waveIndex + 1} / 3 · {tutorialTrainingGoals[waveIndex].label}</span><h2>{wave.title}</h2><p><b>{wave.dataName}</b> · 规模 {Array.from({ length: 5 }, (_, index) => index < wave.size ? '★' : '☆').join('')}<br />分布：{wave.distribution}<br />主要访问：{wave.access}{wave.publicData ? ' · 公共数据' : ''}</p><small>当前负载：{loads.map((load, index) => `DN-${index + 1} ${load}%`).join(' · ')}</small></div>
}
