...
export function LeaderboardPage({ records, onHome, onRestart }: LeaderboardPageProps) {
  ...
  const visibleRecords = useMemo(() => {
    ...
    return (view === 'today' ? rankedRecords.filter((record) => record.dailySeed === seedLabel) : rankedRecords).sort(compareRecords)
  }, [rankedRecords, view])
  return <main className="ranking-page">
    ...
    <section className="ranking-shell">
      <header className="ranking-heading"><div><TrophyIcon /><span>同一 Daily Seed · 本机记录</span></div><h1>数据调度榜</h1><p>按总分排序；同分时比较 PERFECT 数量、最大 Combo、完成时间和平均决策速度。联网总榜接入后可复用同一记录协议。</p></header>
      <div className="ranking-tabs" role="tablist"><button className={view === 'today' ? 'is-active' : ''} onClick={() => setView('today')}>今日榜</button><button className={view === 'overall' ? 'is-active' : ''} onClick={() => setView('overall')}>总榜</button><span>我的最佳：{myBest(rankedRecords) ?? '—'}</span></div>
...
const compareRecords = (a: GameRecord, b: GameRecord) => b.score - a.score
  ...
  || (a.breakdown.averageDecisionMs ?? 0) - (b.breakdown.averageDecisionMs ?? 0)
