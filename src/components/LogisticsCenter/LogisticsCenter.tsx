...
export function LogisticsCenter({ state, preview = false, replicaDataset = 'public' }: Props) {
  ...
  const broadcast = mode === 'query' && (state.queryNodes || 3) !== 1
  const splitting = mode === 'write' || mode === 'final'
  const color = mode === 'query' ? '#FF9F43' : replication ? '#FFD166' : '#32C7F4'
  const skew = Math.max(...state.dnLoads) - Math.min(...state.dnLoads) > 35
  const action = sceneActionFor(mode, state.queryNodes, replicaDataset)
  return <section className={`dispatch-scene ${preview ? 'preview' : ''}`} aria-label="OpenTenBase 数据物流中心">
    <svg viewBox="0 90 1920 730" role="img" aria-label={`CN 调度到三个 DN，负载 ${state.dnLoads.join('、')}%。${skew ? '检测到数据倾斜' : ''}`}>
      ...
      <g className={`scene-action tone-${action.tone}`}>
      ...
      {routes.map((path, i) => { const lit = active && !sync && (mode !== 'query' || (state.queryNodes || 3) !== 1 || i === 1); return <g key={path}>
        <path d={path} fill="none" stroke="#16344b" strokeWidth="8"/>
        <path className={lit ? (broadcast ? 'output-live is-broadcast' : 'output-live') : ''} style={broadcast ? undefined : { animationDelay: `${i * .18}s` }} d={path} fill="none" stroke={lit ? color : '#264862'} strokeWidth="3" opacity={lit ? .8 : .5}/>
        {lit && !reduce && <g key={`${mode}-${i}`}>
          {mode === 'query' ? <circle r="6" fill={color}><animateMotion path={path} dur="1.2s" repeatCount="indefinite"/></circle> : <rect x={replication ? -17 : -13} y={replication ? -12 : -9} width={replication ? 34 : 26} height={replication ? 24 : 18} rx="2" fill={color}><animateMotion path={path} dur={replication ? '.9s' : Math.max(...state.dnLoads) >= 80 ? '.7s' : '1.2s'} begin={replication ? '.7s' : '0s'} repeatCount={replication ? '1' : 'indefinite'} fill="freeze"/></rect>}
        </g>}
      </g> })}
      <g className={`coordinator ${active ? 'active' : ''}`}><rect x="760" y="250" width="400" height="160" rx="6" fill="#0D2138" stroke="#32C7F4" strokeWidth="2"/>
        ...
        {broadcast && !reduce && [-1, 0, 1].map(i => <circle key={`bc-${i}`} r="7" fill="none" stroke="#FF9F43" className="broadcast-flash"><animateMotion path={`M960 320Q${960 + i * 70} 360 960 412`} begin="0s" dur=".5s" fill="freeze"/></circle>)}
