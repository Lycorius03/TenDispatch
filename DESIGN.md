...
决策时看得见的一层只用人话，术语退到第二行与结算。按钮固定为物流动作：按编号分流 / 按地区分流 / 按时间分流 / 按状态分流、就近复制一份、只放一个仓、三个仓都问一遍；副文案只写后果（查询打几个仓、要不要多搬一次）。任务卡只有两行：① 这批货长什么样、② 待会怎么找。Shard / Replication 等名词只出现在教学气泡、按钮第二行与结算的「本波对应」中。结算对照官方用法而不重复报分：大表、持续写入 → 分流、只存一份（分片表）；小而公共、谁都读 → 每仓一份（复制表）；查询条件和分流键不一致 → 触达节点从 1 变成 3。GTM 全程自动播放，不作为玩家选项，结算只用一句人话说明它在对齐事务。
