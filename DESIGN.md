...
写入为方形货物；查询为橙色圆点，一节点查询只点亮中间 DN 轨道；广播查询让三条橙色轨道与三个 DN 落点同时亮起；分流为青色货物在 CN 拆成三份沿三条轨出发；复制以金色副本分裂和三个 DN 到达标记表现；同步模式显示 GTM 波纹及连接线。场景左上角固定一块人话说明牌，写清此刻正在发生什么，空闲状态保持轨道与节点可读。

### 决策层用词

决策时看得见的一层只用人话，术语退到第二行与结算。按钮固定为物流动作：按编号分流 / 按地区分流 / 按时间分流 / 按状态分流、就近复制一份、只放一个仓、三个仓都问一遍；副文案只写后果（查询触达几个 DN、要不要多存两份）。任务卡三行：① 分布、② 查询、③ 本波权重（查询 / 写入 / 读取哪一项更重）。第三行是唯一的优先级声明，保证前两行不会指向不同的动作；同一份负载模型同时驱动按钮数字、预测面板与评分，对不上分流键的动作在节点负载上同样更差。Shard / Replication 等名词只出现在教学气泡、按钮第二行与结算的「本波对应」中。结算对照官方用法而不重复报分：大表、持续写入 → 分流、只存一份（分片表）；小而公共、高频读取 → 每仓一份（复制表）；查询条件和分流键不一致 → 触达节点从 1 变成 3。GTM 全程自动播放，不作为玩家选项，结算只用一句话说明它在对齐事务。
