...
assert.ok(rankedChoice.includes('本局目标：撑完 14 波后，让三个 DN 都保持低负载'))
...
assert.ok(decisionGuide.includes('撑完 14 波，三个 DN 都别接近满载'))
assert.ok(decisionGuide.includes('① 先淘汰不对路的方案'))
assert.ok(decisionGuide.includes('② 再比较选项卡的三行数字'))
assert.ok(decisionGuide.includes('查询 1 DN'))
assert.ok(decisionGuide.includes('搬运低'))
assert.ok(decisionGuide.includes('成本低'))
assert.ok(decisionGuide.includes('③ 最后守住最高负载'))
assert.ok(decisionGuide.includes('宁可多一点小成本，也别让一个 DN 先冲高'))
