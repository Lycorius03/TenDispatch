...
assert.ok(rankedChoice.includes('本局目标：撑完 14 波后，让三个 DN 都保持低负载。比较选项卡下的查询、搬运、成本，选择综合负担最小的方案'))
...
assert.ok(decisionGuide.includes('只看选项卡的三项数字'))
assert.ok(decisionGuide.includes('选择综合负担最小的方案'))
assert.ok(decisionGuide.includes('查询、搬运、成本'))
assert.ok(decisionGuide.includes('三项合起来最小、最划算的方案'))
