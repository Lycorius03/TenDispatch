...
assert.ok(tutorialWave.includes('① 分布'))
assert.ok(tutorialWave.includes('② 查询'))
assert.ok(tutorialWave.includes('③ 当前负载'))
assert.ok(tutorialWave.includes('Shard · 分片表'))
assert.ok(!tutorialWave.includes('查询：1 / 2 / 3 DN'))
assert.ok(!tutorialWave.includes('死亡条件'))
const tutorialWaveReview=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...createTutorialState(),screen:'game',tutorialStep:6,tutorialWaveIndex:0,tutorialWaveCompleted:true,tutorialStrategy:'status'}}))
assert.ok(tutorialWaveReview.includes('本波更合适的动作'))
assert.ok(tutorialWaveReview.includes('按人找就按人分流，否则三个仓都要问。'))
assert.ok(tutorialWaveReview.includes('三个动作的结果'))
assert.ok(tutorialWaveReview.includes('按状态或热点地区分流，数据会集中到一个仓。'))
...
assert.ok(tutorialRouting.includes('查询：'))
...
assert.ok(rankedChoice.includes('本局目标：撑完 14 波后，让三个 DN 都保持低负载。比较选项卡下的查询、搬运、成本，选综合负担最小的动作'))
assert.ok(rankedChoice.includes('① 分布'))
assert.ok(rankedChoice.includes('② 查询'))
assert.ok(rankedOptionCards.includes('class="option-term"'))
for (const option of getRankedWave(0,'TD-CARD-AUDIT').options) {
  assert.ok(rankedOptionCards.includes(option.label), option.label)
  assert.ok(rankedOptionCards.includes(strategyVocabulary[option.id].term), option.id)
}
...
const decisionGuide=renderToStaticMarkup(React.createElement(DecisionDataGuide))
assert.ok(decisionGuide.includes('本题计时已暂停'))
assert.ok(decisionGuide.includes('暂停期间不能选择或确认策略'))
assert.ok(decisionGuide.includes('先读两行，再比三项数字'))
assert.ok(decisionGuide.includes('三张动作卡'))
assert.ok(decisionGuide.includes('查询、搬运、成本'))
assert.ok(decisionGuide.includes('判断依据'))
assert.ok(decisionGuide.includes('触达节点从 1 变成 3'))
assert.ok(!decisionGuide.includes('① 先淘汰不对路的方案'))
assert.ok(!decisionGuide.includes('TD-CARD-AUDIT'))
...
const rankedWaveScore={loadBalance:22,queryEfficiency:4,resourceCost:11,decisionSpeed:10,total:47}
const settledReplica=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...createRankedState('TD-SETTLE'),screen:'game',phase:'ranked-result',waveIndex:0,waveResults:[{wave:2,strategy:'replicated',publicData:false,preferred:false,timedOut:false,decisionMs:900,score:rankedWaveScore,grade:'POOR',combo:0,multiplier:1,earnedScore:47,queryNodes:1,crossNodeMovement:26,loads:[70,72,71],note:'大表全量复制：写入和货位都变成三倍。',predictionUsed:false}]}}))
assert.ok(settledReplica.includes('本波对应：Replication · 复制表'))
assert.ok(settledReplica.includes('就近复制一份'))
assert.ok(settledReplica.includes('查询只触达一个节点，但多存两份，写入要同步三份。'))
assert.ok(settledReplica.includes('官方用法：小而公共、高频读取 → 每仓一份（复制表）'))
assert.ok(settledReplica.includes('多仓一起改的时候，GTM 在对齐事务。'))
const settledBroadcast=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...createRankedState('TD-SETTLE'),screen:'game',phase:'ranked-result',waveIndex:0,waveResults:[{wave:2,strategy:'broadcast',publicData:false,preferred:false,timedOut:false,decisionMs:900,score:rankedWaveScore,grade:'POOR',combo:0,multiplier:1,earnedScore:47,queryNodes:3,crossNodeMovement:40,loads:[70,72,71],note:'广播查询：触达节点从 1 变成 3，搬运同时抬高。',predictionUsed:false}]}}))
assert.ok(settledBroadcast.includes('本波对应：Broadcast · 广播查询'))
assert.ok(settledBroadcast.includes('查询条件和分流键不一致：触达节点从 1 变成 3。'))
assert.ok(settledBroadcast.includes('官方用法：大表、持续写入 → 分流、只存一份（分片表）'))
const broadcastScene=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...initial,phase:'query-result',screen:'game',cargoMode:'query',queryNodes:3,dnLoads:[60,62,61]}}))
assert.ok(broadcastScene.includes('查询：三个 DN 同时被访问'))
assert.ok(broadcastScene.includes('三个 DN 都要参与，结果还要合并'))
assert.equal((broadcastScene.match(/参与本次查询/g)||[]).length,3)
const singleScene=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...initial,phase:'query-result',screen:'game',cargoMode:'query',queryNodes:1,dnLoads:[60,62,61]}}))
assert.ok(singleScene.includes('查询：只触达一个 DN'))
assert.equal((singleScene.match(/本次查询只落这里/g)||[]).length,3)
const splitScene=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...initial,phase:'sharding-result',screen:'game',cargoMode:'write'}}))
assert.ok(splitScene.includes('分流：CN 拆成三份，沿三条轨写入'))
assert.ok(splitScene.includes('shard-splinter'))
const replicaScene=renderToStaticMarkup(React.createElement(GamePage,{...props,state:{...initial,phase:'replication-large-result',screen:'game',cargoMode:'replicate'}}))
assert.ok(replicaScene.includes('复制：三个 DN 各存一份'))
