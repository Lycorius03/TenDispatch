...
assert.ok(tutorialWave.includes('① 这批货长什么样'))
assert.ok(tutorialWave.includes('② 待会怎么找'))
...
assert.ok(tutorialWaveReview.includes('铁律：按人找就按人分流，否则三个仓都要问。'))
assert.ok(tutorialWaveReview.includes('三个动作会把货送到哪里'))
assert.ok(tutorialWaveReview.includes('按状态或热点地区分流，会把数据挤爆一个仓。'))
...
assert.ok(tutorialRouting.includes('待会怎么找：'))
...
assert.ok(rankedChoice.includes('① 这批货长什么样'))
assert.ok(rankedChoice.includes('② 待会怎么找'))
...
assert.ok(decisionGuide.includes('先读两行话，再比三项数字'))
assert.ok(decisionGuide.includes('三张动作卡怎么读'))
assert.ok(decisionGuide.includes('查询、搬运、成本'))
assert.ok(decisionGuide.includes('三条铁律'))
...
assert.ok(settledReplica.includes('查询很快，但多占两份货位，写入要同步三份。'))
assert.ok(settledReplica.includes('官方用法：小而公共、谁都读 → 每仓一份（复制表）'))
...
assert.ok(broadcastScene.includes('查询：三个仓都问一遍'))
assert.ok(broadcastScene.includes('三条橙点同时亮'))
assert.equal((broadcastScene.match(/被问过一次/g)||[]).length,3)
...
assert.ok(singleScene.includes('查询：只打一个仓'))
assert.equal((singleScene.match(/只问这里/g)||[]).length,3)
...
assert.ok(splitScene.includes('分流：一批货从 CN 沿三条轨拆开'))
...
assert.ok(replicaScene.includes('复制：一份货在三个 DN 各落一份'))
...
assert.ok(syncScene.includes('GTM：多仓一起改，在对齐事务'))
