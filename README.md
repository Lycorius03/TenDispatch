...
- 决策时只用人话：任务卡两行（这批货长什么样、待会怎么找），按钮固定成物流动作（按编号 / 地区 / 时间 / 状态分流、就近复制一份、只放一个仓、三个仓都问一遍），Shard / Replication 只在按钮第二行和结算的「本波对应」出现。
...
- Wave 12～14 是 Final Rush，只是同一套铁律的加压，不增加新规则；GTM 在场景中自动完成事务协调动画。
...
- `src/pages/Game/GamePage.tsx`：教学与 Ranked 调度界面、任务卡两行、实时指标和预测面板。
- `src/config/gameConfig.ts`：动作与术语对照表、官方用法三条、教学三波锁定的铁律。
